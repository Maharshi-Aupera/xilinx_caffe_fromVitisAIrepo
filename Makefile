...
# Use static libraries for ./tools to generate ./build/static/
STATIC_LIBRARIES += protobuf
...
# DPU accuracy configuration
ifeq ($(DPU_ACCURACY), 1)
	COMMON_FLAGS += -DDPU_ACCURACY
endif	
...
# Use STATIC_LDFLAGS to link STATIC_LIBRARIES
FILTERED_LIBRARIES = $(filter-out protobuf, $(LIBRARIES))
STATIC_LDFLAGS := $(LDFLAGS) $(foreach librarydir,$(LIBRARY_DIRS),-L$(librarydir)) $(PKG_CONFIG) \
		-Wl,-Bstatic $(foreach library,$(STATIC_LIBRARIES),-l$(library)) -Wl,-Bdynamic $(foreach library,$(FILTERED_LIBRARIES),-l$(library)) 
...
$(STATIC_TOOL_BINS): $(STATIC_TOOL_DIR)/% : $(TOOL_BUILD_DIR)/%.o | $(STATIC_NAME)
	@ echo CXX/LD -o $@
	$(Q)$(CXX) $< -o $@ $(STATIC_LINK_COMMAND) $(STATIC_LDFLAGS)
